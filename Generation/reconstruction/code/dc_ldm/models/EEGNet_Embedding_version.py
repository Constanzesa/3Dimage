...
class EEGNet_Embedding(base_model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(f"this is the size {out.size} ")
        n_out_virtual_chans = out.cpu().data.numpy().shape[2]
        if self.final_conv_length == "auto":
            n_out_time = out.cpu().data.numpy().shape[3]
            self.final_conv_length = n_out_time
        self.embedding = nn.Linear(self.F2*16, 512) #Change 15 here to 128 since input_window_samples = 5096/32
    ...
    def forward(self, x):
        x = self.partial_forward(x) # [1 16 1 15] #bs x F2 x 1 x 15 (128*15) [38,128,1,128]
        x = x.flatten(start_dim=1) # bs x F2*15 [38,16384]
...
#             n_out_time = out.cpu().data.numpy().shape[3]
#             self.final_conv_length = n_out_time
#         self.embedding = nn.Linear(self.F2*128, 512)
#         self.classifier = nn.Linear(512, self.n_classes)
#         self.softmax = nn.LogSoftmax(dim=1)
#         # Transpose time back in third dimension (axis=2)
#         self.permute_back = Expression(_transpose_1_0)
