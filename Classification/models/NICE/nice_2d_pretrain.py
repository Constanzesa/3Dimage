...
#  mat1 and mat2 shapes cannot be multiplied (78x121760 and 1440x768)
...
class Proj_eeg(nn.Sequential):
    def __init__(self, embedding_dim=1440, proj_dim=768, drop_proj=0.5): # 1440
        super().__init__(
...
class IE():
    def __init__(self, args, nsub):
        ...
        self.proj_dim = 256  # We had to resample to 256Hz
        ...
        self.start_epoch = 0
        self.eeg_data_path = 'F:/workspace/NICE-EEG/data/Things-EEG2/Preprocessed_data_250Hz/'
        self.eeg_data_path_fluid = 'F:/workspace/NICE-EEG/data/Fluid/latest/'
        self.img_data_path = 'F:/workspace/NICE-EEG/dnn_feature/' # THINGS embeddings
        self.img_data_path_fluid = './image_embeddings/' # Our embeddings
        self.test_center_path = './image_embeddings/' # Our embeddings centroids
        self.pretrain = False # ?
    ...
    def get_eeg_data(self):
        ...
        train_label = []
        test_data = []
        test_label = labels = np.load(self.eeg_data_path_fluid  + 'group_P03_2D.npy') # np.arange(200)
        ...
        test_data = np.load(self.eeg_data_path_fluid  + 'eeg_P03_2D.npy')
        # /!\ data is structured as (trial, time, channel) while we want (trial, channel, time)
        test_data = test_data.transpose(0, 2, 1)

        # Resample the data from 512 Hz to 256 Hz
        n_samples = test_data.shape[-1] // 2  # Divide the number of samples by 2 to downsample
        downsampled_data = resample(test_data, n_samples, axis=-1)
        test_data = downsampled_data

        #expected input[1, 78, 1, 32]
        #expected input[1, 78, 1, 7680]
    
        #test_data = test_data['preprocessed_eeg_data']
        #test_data = np.mean(test_data, axis=1)  # cf NICE paper, maybe remove
        test_data = np.expand_dims(test_data, axis=1) # equiv unsqueeze
        ...
        # THINGS-EEG was (200, 1, 63, 250)
        #  our (78, 1, 7680)
        # our now  (78, 1, 64, 7680)

        return train_data, train_label, test_data, test_label

    def get_image_data(self):
        train_img_feature = np.load(self.img_data_path + self.args.dnn + '_feature_maps_training.npy', allow_pickle=True)
        test_img_feature = np.load(self.img_data_path_fluid + self.args.dnn + '_feature_maps_training.npy', allow_pickle=True) # this is test feature map: we use here all our dataset as test

        print("Shape before :", train_img_feature.shape, test_img_feature.shape )

        train_img_feature = np.squeeze(train_img_feature)
        test_img_feature = np.squeeze(test_img_feature)

        print("Shape after :", train_img_feature.shape, test_img_feature.shape )
    ...
    def train(self):
        ...
        test_center = np.load(self.test_center_path + 'centroids.npy', allow_pickle=True)
       ...
       # Assuming Proj_eeg is already defined as above with a different embedding_dim
        new_embedding_dim = 121760  # Change this for the test set

        # Instantiate the model with the new embedding_dim for testing
        self.Proj_eeg = Proj_eeg(embedding_dim=new_embedding_dim, proj_dim=768, drop_proj=0.5).cuda()

        # Load pre-trained weights, but ignore the first layer since it has a different size
        if os.path.exists('./model/' + model_idx + 'Proj_eeg_cls.pth'):
            state_dict = torch.load('./model/' + model_idx + 'Proj_eeg_cls.pth')

            # Get the weights for the layers excluding the first linear layer
            filtered_state_dict = {k: v for k, v in state_dict.items() if '0.weight' not in k and '0.bias' not in k}
            
            # Load the filtered weights (excluding the first layer)
            self.Proj_eeg.load_state_dict(filtered_state_dict, strict=False)
        ...
        with torch.no_grad():
            ...
            for i, (teeg, tlabel) in enumerate(self.test_dataloader):
                ...
                all_center = Variable(all_center.type(self.Tensor))  # Error here           
                print("Encoder output: ", self.Enc_eeg(teeg))

                # Apply the Proj_eeg layer to the new test data
                tfea = self.Proj_eeg(self.Enc_eeg(teeg))  # Adjusted model to handle new dimension
        # Here different dimension embedding_dim=121760
