...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == "fit" or stage is None:
            ...
            self.train_dataset = Dataset_Large(Path(self.data_dir), label="label", train=True, special=self.special)
            self.val_dataset = Dataset_Large(Path(self.val_run), label="label", train=False, special=self.special)

        if stage == "test" or stage is None:
            if self.test_dir:
                self.test_dataset = Dataset_Large(Path(self.test_dir), label="label", train=False, special=self.special)
